...
class AuthController extends BaseController
{
    ...
    public function register(array $params = []): void
    {
        ...
        // Check if email already exists. Normalize first so case/whitespace variants
        // (e.g. "John@Example.com" vs "john@example.com") are treated as the same address.
        $normalizedEmail = Validator::normalizeEmail($data['email']);
        if ($this->userModel->emailExists($normalizedEmail)) {
    ...
    public function login(array $params = []): void
    {
        ...
        // Normalize email (lowercased) so users can log in regardless of how
        // they typed it; matches the normalization applied at registration.
    ...
    /**
     * Lightweight email-availability check for the signup form.
     * Returns { available: bool } — never reveals user data.
     * Validates email shape so we don't run a DB lookup on garbage input.
     *
     * GET /api/auth/check-email?email=...
     */
    public function checkEmail(array $params = []): void
    {
        $raw = isset($_GET['email']) ? (string)$_GET['email'] : '';
        if ($raw === '') {
            Response::error('email parameter is required', 400);
        }

        $email = Validator::normalizeEmail($raw);

        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            // Mirror the "not available" shape so the frontend can stay simple,
            // but flag invalid so it can show the right message.
            Response::success(['available' => false, 'invalid' => true], 'Invalid email format');
        }

        $exists = $this->userModel->emailExists($email);
        Response::success(['available' => !$exists], 'OK');
    }
