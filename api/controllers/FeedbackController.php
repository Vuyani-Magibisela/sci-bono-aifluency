...
class FeedbackController extends BaseController
{
    ...
    public function updateStatus(array $params = []): void
    {
        ...
        $adminResponse = !empty($data['admin_response']) ? Validator::sanitize($data['admin_response']) : null;

        $this->executeWithErrorHandling(function () use ($id, $newStatus, $adminNotes, $adminResponse, $currentUser) {
            // Fetch existing feedback + submitting user details before updating
            $fetchStmt = $this->pdo->prepare("
                SELECT f.user_id, f.message, f.contact_email,
                       u.name AS user_name, u.email AS user_email
                FROM user_feedback f
                LEFT JOIN users u ON f.user_id = u.id
                WHERE f.id = ?
            ...
            $stmt = $this->pdo->prepare("
                ...
                SET status = ?, admin_notes = ?, admin_response = ?, resolved_by = ?, resolved_at = ?
                WHERE id = ?
            ");
            $stmt->execute([$newStatus, $adminNotes, $adminResponse, $resolvedBy, $resolvedAt, $id]);

            // --- In-app notification ---
            // Only notify authenticated users and only for meaningful status transitions
            $notifyStatuses = ['in_review', 'resolved'];
            $userId = $existing['user_id'] ?? null;

            if ($userId && in_array($newStatus, $notifyStatuses)) {
                $statusLabels = ['in_review' => 'Under Review', 'resolved' => 'Resolved'];
                $notifTitle = 'Your feedback has been ' . $statusLabels[$newStatus];
                $notifMessage = $adminResponse
                    ? 'The team replied: ' . mb_substr($adminResponse, 0, 120) . (mb_strlen($adminResponse) > 120 ? '…' : '')
                    : 'Your feedback status was updated to ' . $statusLabels[$newStatus] . '.';

                try {
                    $notifStmt = $this->pdo->prepare("
                        INSERT INTO notifications (user_id, type, title, message, link)
                        VALUES (?, 'feedback_update', ?, ?, '/sci-bono-aifluency/public/dashboard.html')
                    ");
                    $notifStmt->execute([$userId, $notifTitle, $notifMessage]);
                } catch (\Exception $e) {
                    error_log("Notification creation failed for feedback {$id}: " . $e->getMessage());
                }
            }

            // --- Email notification ---
            // Send email when admin has written a response and status is meaningful
            $recipientEmail = $existing['user_email'] ?? $existing['contact_email'] ?? null;
            $recipientName  = $existing['user_name'] ?? 'there';

            if ($recipientEmail && $adminResponse && in_array($newStatus, $notifyStatuses)) {
                $siteUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http')
                         . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost')
                         . '/sci-bono-aifluency/public/dashboard.html';

                $subject  = 'Update on your feedback – Sci-Bono AI Hub';
                $htmlBody = Mailer::buildFeedbackUpdateEmail(
                    $recipientName,
                    $newStatus,
                    $adminResponse,
                    $existing['message'],
                ...
                try {
                    Mailer::send($recipientEmail, $subject, $htmlBody);
                } catch (\Throwable $e) {
                    // Log but don't fail the request — email is supplementary
                    error_log("Feedback email failed for feedback {$id}: " . $e->getMessage());
                }
            }
